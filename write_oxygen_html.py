...
for i, genus in enumerate(genera):
    for j, spec in enumerate(species[i]):
        ...
        edges.append({"id":"se"+str(i)+str(j), "source":"gn"+str(i), "target":"sn"+str(i)+str(j),"size":20,"color":grey})
    o = g_oxygen[i]
    if o == "aerobe":
        color = redorange
            ...
            if lin[5] == genus:
                ...
                for j, n in enumerate([4, 3, 2, 1, 0]):
                    ...
                    if not any(node["label"] == lin[n] for node in nodes):
                        nodes.append({"id":"tn"+str(n)+str(i), "label":lin[n], "x":xs[j], "y":ys[j], "size":sizes[j], "color":dark_grey})
                #for each node
                for node in nodes:
                    #if its label belongs to the genus' family, connect the node to the genus
                    if node["label"] == lin[4]:
                        edges.append({"id":"fe"+str(i), "source":"gn"+str(i), "target":node["id"],"size":20,"color":grey})
                        ...
                        for node in nodes:
                            ...
                            if node["label"] == lin[3]:
                                edges.append({"id":"oe"+str(i), "source":source, "target":node["id"],"size":20,"color":grey})
                                #mark the order node
                                source = node["id"]
                                #for each node
                                for node in nodes:
                                    ...
                                    if node["label"] == lin[2]:
                                        edges.append({"id":"ce"+str(i), "source":source, "target":node["id"],"size":20,"color":grey})
                                        #mark the class node
                                        source = node["id"]
                                        #for each node
                                        for node in nodes:
                                            ...
                                            if node["label"] == lin[1]:
                                                edges.append({"id":"pe"+str(i), "source":source, "target":node["id"],"size":20,"color":grey})
                                                #mark the class node
                                                source = node["id"]
                                                #for each node
                                                for node in nodes:
                                                    ...
                                                    if node["label"] == lin[0]:
                                                        edges.append({"id":"de"+str(i), "source":source, "target":node["id"],"size":20,"color":grey})
                break
